...
public class Trail extends PolyLine {
	...
	protected int curveLength = 20;
	
	protected int pointsBegining = 0;
	protected double refreshRate = 0.1;
	...
	void setCoords() {
        ...
        Vector2f d;
        d = new Vector2f(points[pointsBegining+0], points[pointsBegining+1]).sub(new Vector2f(points[pointsBegining+2], points[pointsBegining+3])).getNormal();
        d.normalize();
        d.mul(thickness);
        ...
        for(int i=1; i<=(pointsCount-pointsBegining)/2-2; ++i){
        	...
        	Vector2f v0 = new Vector2f(points[pointsBegining+2*i-2], points[pointsBegining+2*i-1]);
        	Vector2f v1 = new Vector2f(points[pointsBegining+2*i  ], points[pointsBegining+2*i+1]);
        	Vector2f v2 = new Vector2f(points[pointsBegining+2*i+2], points[pointsBegining+2*i+3]);
        ...
        d = new Vector2f(points[offset-4], points[offset-3])
        		.sub(new Vector2f(points[offset-2], points[offset-1])).getNormal();
        d.normalize();
        d.mul(thickness/1.f);
